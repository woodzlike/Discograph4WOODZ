...
> 아래 항목은 Shrimp Task Manager 기준 의도적으로 보류한 작업이다. Phase 3·4의 작업 목록과 완료 기준, PRD의 MVP 체크리스트에는 여전히 문구가 남아 있지만, 별도 요청이 있기 전까지 구현하지 않는다.

- **앨범 상세 페이지** (`app/album/[id]/page.tsx`) — 앨범 커버/제목/발매일/레이블, 트랙리스트, Spotify 링크 버튼, Notion 팬 노트 섹션. Phase 3 완료 기준의 "트랙리스트가 렌더링된다"·"팬 노트 섹션이 표시된다" 항목과 PRD MVP 체크리스트의 "앨범 클릭 시 트랙리스트 상세 페이지"·"추천 배지·팬 노트 오버레이"가 이 페이지에 의존한다.
- **UX 보완** (Phase 4) — 앨범 그리드 Skeleton UI, 에러 바운더리, 반응형 그리드 다듬기, `StarRating` 소수점 반별 처리
- **OG 메타데이터 및 뒤로가기 버튼** (Phase 4) — 앨범 상세 페이지가 없는 상태이므로 함께 보류

남은 미완료 작업은 **Vercel 배포**뿐이며, 이는 제외가 아니라 아직 진행하지 않은 별개 작업이다.
...
- [x] Next.js Server Component로 API 호출 처리 (클라이언트에 키 노출 방지)
...
- [x] Next.js `revalidate` 설정 — Spotify/Notion 데이터 캐싱 주기 설정 (예: 1시간)
- [x] `next/image` — 앨범 커버 이미지 최적화 (`priority`, `sizes` 설정)
- [x] `npm run build` 성공 확인 및 TypeScript / ESLint 오류 0건 확인
- [x] Vercel 배포 — 환경 변수 세팅 후 프로덕션 배포
- [x] 배포 후 실제 URL에서 전체 플로우 동작 확인
...
- [x] Spotify API로 WOODZ 전체 앨범 자동 수집
- [x] 메인 앨범 카드 그리드 렌더링
- [x] 앨범 타입 필터 (정규 / EP / 싱글)
- [ ] 앨범 클릭 시 트랙리스트 상세 페이지 (제외된 작업 — 위 "이번 범위에서 제외된 작업" 참고)
- [ ] Notion API 연동 및 추천 배지·팬 노트 오버레이 (추천 배지는 메인 페이지에 반영됨. 팬 노트는 앨범 상세 페이지 제외로 보류)
